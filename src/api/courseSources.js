...
export async function uploadCourseSourceFile(job, file, onProgress = () => {}) {
    ...
    try {
        ...
        onProgress(`${file.name} uploaded securely.`);
        return document;
    } catch (error) {
        if (fileUploaded) {
            await supabase.storage.from('course_sources').remove([storagePath]).catch(() => {});
        }
        throw error;
    }
}
